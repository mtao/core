...
constexpr bool col_check_with_assert(const Eigen::MatrixBase<Derived>& N, std::integer_sequence<int,Cs...>) {
...
constexpr bool row_check_with_assert(const Eigen::MatrixBase<Derived>& N, std::integer_sequence<int,Rs...>) {
...
constexpr bool shape_check_with_assert(const Eigen::MatrixBase<Derived>& N, std::integer_sequence<int,Rs...> rows, std::integer_sequence<int,Cs...> cols) {
    return row_check_with_assert<Rs...>(N,rows) && col_check_with_assert<Cs...>(N,cols);
}

// These functions return in case asserts are disabled
template <int... Cs, typename Derived>
constexpr bool col_check(const Eigen::MatrixBase<Derived>& N, std::integer_sequence<int,Cs...>) {
    constexpr int cols = Derived::ColsAtCompileTime;
    if constexpr (cols == Eigen::Dynamic) {
...
template <int... Rs, typename Derived>
constexpr bool row_check(const Eigen::MatrixBase<Derived>& N, std::integer_sequence<int,Rs...>) {
    constexpr int rows = Derived::RowsAtCompileTime;
    if constexpr (rows == Eigen::Dynamic) {
        bool value = ( ( N.rows() == Rs) || ... || true) ;
        assert(value);
...
template <int... Rs, int... Cs, typename Derived>
constexpr bool shape_check(const Eigen::MatrixBase<Derived>& N, std::integer_sequence<int,Rs...> rows, std::integer_sequence<int,Cs...> cols) {
