...
namespace detail {
    ...
    template<bool UseRow, int... N, typename Derived, typename IndexContainer = mtao::types::empty_t>
    auto _partition_inner_stride_writable(std::integer_sequence<int, N...>,
        ...
        if constexpr (UseRow) {
            if constexpr (Derived::ColsAtCompileTime == 1) {
                ...
                return std::make_tuple(
                  // M.head(std::get<0>(indices),
                  M.block(std::get<N>(indices), start, std::get<N + 1>(indices) - std::get<N>(indices), end)...);
            }
        } else {
            if constexpr (Derived::RowsAtCompileTime == 1) {
                ...
                return std::make_tuple(
                  // M.head(std::get<0>(indices),
                  M.block(start, std::get<N>(indices), end, std::get<N + 1>(indices) - std::get<N>(indices))...);
            }
        }
    }

    template<bool UseRow, int... N, typename Derived, typename RowIndexContainer, typename ColIndexContainer>
            ...
            if constexpr (std::is_same_v<RowIndexContainer, types::empty_t>) {
                ...
                return std::make_tuple(_partition_inner_stride_writable<false>(
                  std::make_integer_sequence<
                    int,
                    std::tuple_size_v<ColIndexContainer> - 1>{},
                  M,
                  cols,
                  std::get<N>(rows),
    ...
    auto _partition_inner_stride(std::integer_sequence<int, N...>,
        ...
        if constexpr (UseRow) {
            if constexpr (Derived::ColsAtCompileTime == 1) {
                ...
                return std::make_tuple(
                  // M.head(std::get<0>(indices),
                  M.block(std::get<N>(indices), 0, std::get<N + 1>(indices) - std::get<N>(indices), M.cols())...);
            }
        } else {
            if constexpr (Derived::RowsAtCompileTime == 1) {
                return std::make_tuple(
                ...
                return std::make_tuple(
                  // M.head(std::get<0>(indices),
                  M.block(0, std::get<N>(indices), M.rows(), std::get<N + 1>(indices) - std::get<N>(indices))...);
            }
        }
    }
            ...
            if constexpr (std::is_same_v<RowIndexContainer, types::empty_t>) {
                ...
                return std::make_tuple(_partition_inner_stride<false>(
                  std::make_integer_sequence<
                    int,
                    std::tuple_size_v<ColIndexContainer> - 1>{},
                  M.block(std::get<N>(rows), 0, std::get<N + 1>(rows) - std::get<N>(rows), M.cols()),
                  cols)...);
            }
        } else {
            if constexpr (std::is_same_v<ColIndexContainer, types::empty_t>) {
                ...
                return std::make_tuple(_partition_inner_stride<true>(
                  std::make_integer_sequence<
                    int,
                    std::tuple_size_v<ColIndexContainer> - 1>{},
                  M.block(0, std::get<N>(cols), M.rows(), std::get<N + 1>(cols) - std::get<N>(cols)),
                  rows)...);
            }
        }
    }
}// namespace detail
template<typename Derived, typename RowIndexContainer = mtao::types::empty_t, typename ColIndexContainer = mtao::types::empty_t>
...
auto partition_matrix_from_sizes(const Eigen::MatrixBase<Derived> &M,
                                 ...
                                 const ColIndexContainer &cols) {
    // get the partition stride with row = true
    return partition_matrix(M, utils::partial_sum(rows), utils::partial_sum(cols));
}
