...
namespace geometry {
    ...
    class KDNode {
      public:
        ...
        void nearest_filtered(const Eigen::MatrixBase<Derived> &p, size_t &nearest_index, T &nearest_dist, FilterFunc &&filter) const {
        ...
        ValueType accumulate(Reduce &&g, const ValueType &value, int depth = 0) const {
    ...
    class KDTree {
      public:
        ...
        std::tuple<size_t, T> nearest_filtered(
          const typename Eigen::MatrixBase<Derived> &p,
          Filter &&filter) const {
            size_t idx = 0;
            T d = std::numeric_limits<T>::max();
            assert(m_node);
        ...
        operator std::string() const {
            ...
            return accumulate<std::string>(
              [](const Vec &, int index, int, const std::string &left_str, const std::string &right_str) {
                  std::stringstream ss;
        ...
        int max_depth() const {
            return accumulate(
                     [](const Vec &, int, int depth, int left_depth, int right_depth) {
