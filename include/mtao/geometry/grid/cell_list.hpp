...
struct CellList : public GridD<Scalar, D> {
    ...
    void neighbor_walker(Func&& f, const Eigen::MatrixBase<Derived>& p,
                         double radius) const;
    template <typename Func>
    void neighbor_walker(Func&& f, const coord_type& c, int radius = 1) const;
    template <typename Func>
    void neighbor_walker(Func&& f, const coord_type& c,
...
template <typename Scalar, int D>
template <typename Func>
void CellList<Scalar, D>::neighbor_walker(Func&& f, const coord_type& c,
                                          int radius) const {
    coord_type r;
