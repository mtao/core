...
auto curve_volume(const Eigen::MatrixBase<Derived>& V, const BeginIt& beginit,
                  const EndIt& endit) -> typename Derived::Scalar {
    auto it = beginit;
    auto it1 = beginit;
    it1++;
    typename Derived::Scalar ret = 0;
...
typename ADerived::Scalar triangle_area(const Eigen::MatrixBase<ADerived>& a,
    ...
    auto l2 = (c - a).norm();
    // auto s = (l0 + l1 + l2) / 2;
    return std::sqrt((l0 + l1 + l2) * (2 * l0 + 2 * l1 - l2) *
                     (2 * l0 - l1 + 2 * l2) * (-l0 + 2 * l1 + 2 * l2)) /
           4;
}
