...
void EmbeddedHalfEdgeMesh<S, D>::set_one_ring_adjacencies_quadcross(
  const Eigen::MatrixBase<Derived> &o,
  const std::set<int> &edges) {
    ...
    if (edges.empty()) {
        ...
        std::transform(order.begin(), order.end(), order.begin(), [&](int idx) { return edges_vec.at(idx); });
        ...
        for (auto &&ihe : interior_hes) {
            ...
            if (ocell == icell || dicell == ocell) {
                ...
                if (is_inside(V, edge(ohe), CM[ihe])) {
                    ...
                    // to find it
                    if (std::includes(
                          potential_parent.begin(), potential_parent.end(), potential_child.begin(), potential_child.end())) {
                        auto &&child = CM[ihe];
                        auto &&parent = CM[ohe];
