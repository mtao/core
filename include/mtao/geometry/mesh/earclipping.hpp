...
std::vector<std::array<int, 3>> earclipping_stl(
  ...
  const EndIt &endit) {
    ...
    if constexpr (std::is_integral_v<
                    mtao::types::remove_cvref_t<decltype(*beginit)>>) {
...
mtao::ColVectors<int, 3> earclipping(const Eigen::MatrixBase<VDerived> &V,
                                     const Container &C) {
