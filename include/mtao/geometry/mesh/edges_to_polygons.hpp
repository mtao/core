...
std::vector<PolygonBoundaryIndices> edges_to_polygons(
  ...
  const std::vector<std::tuple<std::vector<int>, bool>> &loops) {
    ...
    for (auto &&node : dag.nodes()) {
        ...
        std::transform(
          node->children.begin(), node->children.end(), std::inserter(pbi.holes, pbi.holes.end()), [&](auto &&ptr) { return std::get<0>(loops.at(ptr->value)); });
...
namespace internal {
    ...
    std::partial_ordering
      winding_number_loop_comparator<Symmetric, VDerived>::operator()(
        ...
        IndSet diff;
        std::set_intersection(as.begin(), as.end(), bs.begin(), bs.end(), std::inserter(diff, diff.end()));
        IndSet auniq, buniq;

        auto is_inside = [&](IndSet &a, const Container &B) -> bool {
            return std::all_of(a.begin(), a.end(), [&](typename Container::value_type index) -> bool {
                return geometry::interior_winding_number(V, B, V.col(index));
            });
        };

        // get the unique indices in either set
        std::set_difference(as.begin(), as.end(), diff.begin(), diff.end(), std::inserter(auniq, auniq.end()));
        std::set_difference(bs.begin(), bs.end(), diff.begin(), diff.end(), std::inserter(buniq, buniq.end()));
