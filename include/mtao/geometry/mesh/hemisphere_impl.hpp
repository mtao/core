#ifndef _HEMISPHERE_MESH_GEN_H_
#define _HEMISPHERE_MESH_GEN_H_
#include <array>
#include <map>
#include <vector>
#include <string>
...
template <typename T>
int HemisphereMeshFactory<T>::add_edge(Edge e) {
    if(e[0] > e[1]) {
        int tmp = e[0];
        e[0] = e[1];
        e[1] = tmp;
...
template <typename T>
void HemisphereMeshFactory<T>::write(const std::string & filename) {
    std::ofstream outstream(filename.c_str());
    write(outstream);
}

template <typename T>
void HemisphereMeshFactory<T>::write(std::ostream & outstream) {
