...
namespace opengl {
    ...
    struct bind_enabled {
      ...
      private:
        template<typename U = Obj>
        constexpr static bool optional_exists() { return optional_exists_<U>(0); }
        //&U::optional has a reference in order to make it a ptr type so passing 0 works
        template<typename U>
        constexpr static bool optional_exists_(decltype(&U::optional)) { return true; }
        //C style varargs to let anything passed in works
        template<typename U>
        constexpr static bool optional_exists_(...) { return false; }
    };
