...
template <typename ShaderType>
using DrawableType =
    std::conditional_t<ShaderDim<ShaderType>() == 2, SceneGraph::Drawable2D,
...
template <typename ShaderType>
using CameraType =
    std::conditional_t<ShaderDim<ShaderType>() == 2, SceneGraph::Camera2D,
                       SceneGraph::Camera3D>;
template <typename ShaderType>
    ...
    virtual void draw(const TransMat& transformationMatrix,
        ...
        if (triangle_primitive) {
            ...
            _mesh.setIndexBuffer(
                _mesh.triangle_index_buffer, 0, _mesh.triangle_indexType,
                _mesh.triangle_indexStart, _mesh.triangle_indexEnd);
            _mesh.setPrimitive(*triangle_primitive);
            shader().draw(_mesh);
        }
        if (edge_primitive) {
            _mesh.setCount(_mesh.edge_Count);
            _mesh.setIndexBuffer(_mesh.edge_index_buffer, 0,
                                 _mesh.edge_indexType, _mesh.edge_indexStart,
                                 _mesh.edge_indexEnd);
        ...
        if (point_primitive) {
            ...
            _mesh.setIndexBuffer(
                _mesh.vertex_index_buffer, 0, _mesh.vertex_indexType,
                _mesh.vertex_indexStart, _mesh.vertex_indexEnd);
