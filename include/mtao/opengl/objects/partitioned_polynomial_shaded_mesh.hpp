...
class PartitionedPolynomialShadedMesh
  ...
  , std::conditional_t<D == 2, SceneGraph::Drawable2D, SceneGraph::Drawable3D> {
  public:
  ...
  public:
    ...
    Corrade::Containers::Array<Magnum::GL::MeshView> &views() { return _views; }
...
void PartitionedPolynomialShadedMesh<D>::draw(
  const TransMat &transformationMatrix,
  Camera &camera) {
    ...
    M::addVertexBuffer(M::vertex_buffer, 0, typename PolynomialScalarFieldShader<D>::Position{});
...
void PartitionedPolynomialShadedMesh<D>::set_offsets(
  const std::vector<int> &offsets) {
    ...
    M::setIndexBuffer(M::triangle_index_buffer, 0, M::triangle_indexType, M::triangle_indexStart, M::triangle_indexEnd);
