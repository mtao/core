...
openvdb::GridBase::Ptr get_grid(const std::string& filename, const std::string& gridname = "") {
    ...
    if(!gridname.empty()) {
        ...
        std::cout << "WARN: did not find the grid " << gridname << " in " << filename << std::endl;
    } else {
...
int main(int argc, char* argv[]) {
    ...
    options.add_options()
        ("vdb_file", "vdb grid file",cxxopts::value<std::string>())
        ...
        ("adaptivity_name", "the name of the adaptivity grid in the adaptivity vdb file",cxxopts::value<std::string>())
        ("q,quads", "Output quads (only for obj)",cxxopts::value<bool>()->default_value("false"))
        ("r,relax_tris", "Relax disoriented triangles",cxxopts::value<bool>()->default_value("true"))
        ("h,help", "Print usage");
    ...
    std::cout << "Checking for adaptivity name and fetching adaptivity" << std::endl;
    if(result.count("adaptivity_file")) {
    std::string adaptivity_file = result["adaptivity_file"].as<std::string>();
        if(result.count("adaptivity_name")) {
            std::string adaptivity_name = result["adaptivity_name"].as<std::string>();
    ...
    auto run = [&](auto&& grid) {
        ...
        for(auto&& [idx,pt]: mtao::iterator::enumerate(mtao::iterator::shell(pts))) {
        }
    };
    ...
    if(grid->type() == "Tree_float_5_4_3") {
        ...
        std::cerr << "Unknown tree type! pass in float or double grids!: " << grid->type() << std::endl;
    }
