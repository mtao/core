...
class MeshViewer : public mtao::opengl::Window3 {
  public:
    ...
    MeshViewer(const Arguments &args)
      : Window3(args),
        ...
        edge_drawable = new mtao::opengl::MeshDrawable<Magnum::Shaders::VertexColor3D>{
        ...
        _plane_viewer = new mtao::opengl::MeshDrawable<Magnum::Shaders::Phong>{ plane, _phong_shader, drawables() };

        update();
    }
    auto get_grid() const {
        return mtao::geometry::grid::Grid3f::from_bbox(
          bbox, std::array<int, 3>{ { NI, NJ, NK } });
    }
    void update_grid_colors() {
        auto g = get_grid();
        auto V = g.vertices();
        mtao::Vec4f P = plane.get_equation();
        auto vals = (P.transpose() * V.colwise().homogeneous()).cwiseAbs();
        colormap::MATLAB::Parula parula;
        mtao::ColVecs4f C(4, vals.size());
        for (auto &&[idx, val] : mtao::iterator::enumerate(vals)) {
            auto c = parula.getColor(val);
            C.col(idx) << c.r, c.g, c.b, 1.0f;
        }
  ...
  private:
    ...
    mtao::opengl::objects::Plane plane;
    ...
    mtao::opengl::MeshDrawable<Magnum::Shaders::VertexColor3D> *edge_drawable = nullptr;
