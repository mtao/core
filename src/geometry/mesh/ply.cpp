...
void PLY::Element::Property::write_header(std::ostream &os) const {
    if (is_list()) {
        fmt::print(os, "property list {} {} {}\n", internal::property_type_name(*index_type), internal::property_type_name(*index_type), name);
    } else {
...
size_t PLY::Element::Property::byte_size(size_t count) const {
    ...
    if (index_type) {
        return internal::single_element_byte_size(*index_type) + count * internal::single_element_byte_size(type);
    } else {
