...
void write_objF(const mtao::ColVectors<float, 3> &V, const std::string &filename) {
    write_obj(V, {}, {}, filename);
}
void write_objD(const mtao::ColVectors<double, 3> &V, const std::string &filename) {
    write_obj(V, {}, {}, filename);
}
}// namespace mtao::geometry::mesh
